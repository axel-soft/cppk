...
Процедура ЦППК_ПолеНадписиВноситИзменения_Нажатие(Элемент)
	...
	Для Каждого Строка Из ЭтаФорма.ЦППК_СписокИзмененныхДокументов Цикл
		ОписаниеДок = Новый Структура("Документ, ДатаОтмены, УжеУстановлен, Источник",
			Строка.Документ, Строка.ДатаИзменения, Строка.УжеУстановлен, Строка.Источник);
...
// Возвращаеемое значение:
//   Строка - имя элемента управления на форме
//
&НаСервереБезКонтекста
Функция ИмяДопРеквизитаНаФорме(ИмяДляФормул)
...
#Вставка
	...
	СписокИзмененныхДокументов = ЭтаФорма.ЦППК_СписокИзмененныхДокументов.Выгрузить(, "Документ").ВыгрузитьКолонку("Документ");
