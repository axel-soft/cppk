...
#Вставка
	//Если ВидДокументаКэш.ЦППК_УчитыватьВноситИзменения Тогда
	//	СписокПолномочий = Новый СписокЗначений;
	//	СписокПолномочий.Добавить(Справочники.ПрофилиГруппДоступа.Администратор);
	//	СписокПолномочий.Добавить(Справочники.ПрофилиГруппДоступа.ПолучитьСсылку(Новый УникальныйИдентификатор("eeb325ab-db51-11de-a544-00179ab398dc")));
	//	Если НЕ РегистрыСведений.ПолномочияСотрудников.ИмеетПолномочия(ПараметрыСеанса.ТекущийПользователь, СписокПолномочий) Тогда
...
Функция ИмяДопРеквизитаНаФорме(ИмяДляФормул)
    ...
    Набор = ДопРеквизит.НаборСвойств;
    УникальнаяЧасть = СтрЗаменить(ВРег(Строка(Набор.УникальныйИдентификатор())), "-", "x")
            + "_"
            + СтрЗаменить(ВРег(Строка(ДопРеквизит.УникальныйИдентификатор())), "-", "x");
...
Процедура ОбновитьСвязиИзмененныхДокументов(ТекущийОбъект, СписокИзмененныхДокументов)
	...
	Пока Выборка.Следующий() Цикл
		...
		ИзмененыйДокумент = Выборка.СвязанныйОбъект.ПолучитьОбъект();
		ИзмененыйДокумент.ЦППК_Изменен = Ложь;
		ИзмененыйДокумент.Записать();
	КонецЦикла;
	...
	Запрос.Текст = 
		...
		|	И НЕ ЦППК_ИзмененныеДокументы.ОтменяемыйДокумент В (&ОтменяемыйДокумент)";
	
	Запрос.УстановитьПараметр("ОтменяемыйДокумент", СписокИзмененныхДокументов);
