...
#Вставка
	Если Элементы.Найти("Изменен") <> Неопределено Тогда
		Элементы.Изменен.Доступность = Ложь;
		Элементы.ИзмененВСоответствии.Доступность = Ложь;
	КонецЕсли;
...
#Вставка
	...
	// Котляров ВВ 07.08.2025 САНФ-034810
...
Процедура ЦППК_ПолеНадписиВноситИзменения_Нажатие(Элемент)
	...
	Для Каждого Строка Из ЭтаФорма.ЦППК_СписокИзмененныхДокументов Цикл
		...
		ОписаниеДок.Вставить("Документ", Строка.Документ);
		ОписаниеДок.Вставить("ДатаОтмены", Строка.ДатаИзменения);
		ОписаниеДок.Вставить("УжеУстановлен", Строка.УжеУстановлен);
		ОписаниеДок.Вставить("Источник", Строка.Источник);
...
Процедура ЦППК_ПослеЗакрытияВводИзмененныхДокументов(Результат, ПараметрыОповещения) Экспорт
	...
	Если Результат <> Неопределено И ТипЗнч(Результат) = Тип("Массив") Тогда
		...
		Для Каждого Строка Из Результат Цикл
			...
			НовСтрока.ДатаИзменения = Строка.ДатаОтмены;
...
Процедура ЦППК_Подключаемый_ПриИзмененииДополнительногоРеквизита(Элемент)
	...
	// Котляров 16.07.2025 САНФ-035024
	Если Элемент.Имя = ИмяДопРеквизитаНаФорме("НачалоСрокаИсполненияОбязательств") Тогда
		ОбработкаИзменения_НачалоСрокаИсполненияОбязательств(Элемент.Имя);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ОбработкаИзменения_НачалоСрокаИсполненияОбязательств(ИмяРеквизита_НачалоСрокаИсполненияОбязательств)
	
	// ++AxelSoft Котляров 06.08.2025 САНФ-035582  Условие изменения «Окончание срока действия обязательств Контрагента»
	ИмяРеквизита_КоличествоДнейДействияДоговора = ИмяДопРеквизитаНаФорме("КоличествоДнейДействияДоговора");
	Если НЕ ЗначениеЗаполнено(ЭтаФорма[ИмяРеквизита_КоличествоДнейДействияДоговора]) Тогда
		Возврат;	
	КонецЕсли;
	// ++AxelSoft Котляров 06.08.2025 САНФ-035582  Условие изменения «Окончание срока действия обязательств Контрагента»
	
	Если ЗначениеЗаполнено(ЭтаФорма[ИмяРеквизита_НачалоСрокаИсполненияОбязательств]) Тогда
		ИмяРеквизита_ТребуетсяАктДоступа = ИмяДопРеквизитаНаФорме("ТребуетсяАктДоступа");
		ИмяРеквизита_ОкончаниеСрокаИсполненияОбязательств = ИмяДопРеквизитаНаФорме("ОкончаниеСрокаИсполненияОбязательств");
		Если ИмяРеквизита_ТребуетсяАктДоступа = "" ИЛИ
			 ИмяРеквизита_КоличествоДнейДействияДоговора = "" ИЛИ 
			 ИмяРеквизита_ОкончаниеСрокаИсполненияОбязательств = "" Тогда
		
			 ВызватьИсключение("Найдены не все необходимые реквизиты для расчета.");		
		КонецЕсли;
		
		Если ЭтаФорма[ИмяРеквизита_ТребуетсяАктДоступа] = Истина Тогда
			СекундВСутках = 86400;			
			ЭтаФорма[ИмяРеквизита_ОкончаниеСрокаИсполненияОбязательств] = ЭтаФорма[ИмяРеквизита_НачалоСрокаИсполненияОбязательств]
																		+ (ЭтаФорма[ИмяРеквизита_КоличествоДнейДействияДоговора] * СекундВСутках);
			Объект.ДатаНачалаДействия = ЭтаФорма[ИмяРеквизита_НачалоСрокаИсполненияОбязательств];
			Объект.ДатаОкончанияДействия = ЭтаФорма[ИмяРеквизита_ОкончаниеСрокаИсполненияОбязательств];
			Объект.ДоИсполненияОбязательств = Ложь;
			Объект.Бессрочный = Ложь; 
			ЭтаФорма["СрокДействияСтрока"] = ДелопроизводствоКлиентСервер.СформироватьПредставлениеСрокаДействия(Объект, "");
		КонецЕсли;
	КонецЕсли;	
КонецПроцедуры
...
//
// Возвращает имя дополнительного реквизита на форме по его идентификатору
// Параметры:
//   ИмяДляФормул - Строка - идентификатор реквизита
// Возвращаемое значение:
//   Строка - имя элемента управления на форме
//
&НаСервереБезКонтекста
Функция ИмяДопРеквизитаНаФорме(ЗНАЧ ИмяДляФормул)
    
    ДополнительныйРеквизит = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту(
        															"ИдентификаторДляФормул", ИмяДляФормул);
    
    Если Не ЗначениеЗаполнено(ДополнительныйРеквизит) ИЛИ Не ЗначениеЗаполнено(ДополнительныйРеквизит.НаборСвойств) Тогда
        Возврат "";
    КонецЕсли;
    
    НаборСвойств = ДополнительныйРеквизит.НаборСвойств;
    УникальнаяЧасть = СтрЗаменить(ВРег(Строка(НаборСвойств.УникальныйИдентификатор())), "-", "x")
            + "_"
            + СтрЗаменить(ВРег(Строка(ДополнительныйРеквизит.УникальныйИдентификатор())), "-", "x");

    Возврат "ДополнительныйРеквизитЗначение_" + УникальнаяЧасть;
КонецФункции
...
Процедура ЦППК_ПрочитатьСвязи(ТипыСвязей)
	...
	ЭтаФорма.ИзмененВСоответствии = СвязиОбъектов.ПолучитьСвязанныйОбъект(Объект.Ссылка, 
	...
	Если ТипыСвязей = Неопределено Или ТипыСвязей.Найти(Справочники.ТипыСвязей.ЦППК_ИзмененДокументом) <> Неопределено Тогда 
		...
		ЭтаФорма.ЦППК_СписокИзмененныхДокументов.Очистить();
		ЦППК_ОбщийМодульВызовСервера.ЦППК_ПрочитатьИзмененныеДокументы(Объект.Ссылка,
																	   Объект.РегистрационныйНомер,
																	   ЭтаФорма.ЦППК_СписокИзмененныхДокументов);
...
#Вставка
	...
	//  Удалить связь из регистра
	СписокИзмененныхДокументов = ЭтаФорма.ЦППК_СписокИзмененныхДокументов.Выгрузить(, "Документ").
																			ВыгрузитьКолонку("Документ");
	ОбновитьСвязиИзмененныхДокументов(ТекущийОбъект.Ссылка, СписокИзмененныхДокументов);
...
Процедура ОбновитьСвязиИзмененныхДокументов(ТекущийОбъект, СписокИзмененныхДокументов)

	Запрос = Новый Запрос;

	Запрос.УстановитьПараметр("ТипСвязи", Справочники.ТипыСвязей.ЦППК_ИзменяетДокумент); 
	Запрос.УстановитьПараметр("ТипСвязиИзменен", Справочники.ТипыСвязей.ЦППК_ИзмененДокументом);
	Запрос.УстановитьПараметр("СвязанныйОбъект", СписокИзмененныхДокументов);
	...
	|ГДЕ
	|	НЕ СвязиОбъектов.СвязанныйОбъект В (&СвязанныйОбъект)
	|	И СвязиОбъектов.Объект = &Объект
	...
	|	РегистрСведений.СвязиОбъектов КАК СвязиОбъектов
	|ГДЕ
	|	НЕ СвязиОбъектов.Объект В (&СвязанныйОбъект)
	|	И СвязиОбъектов.СвязанныйОбъект = &Объект
	|	И СвязиОбъектов.ТипСвязи = &ТипСвязиИзменен
	|";
	...
	Пока Выборка.Следующий() Цикл
		...
		ИзмененныйДокумент = Выборка.СвязанныйОбъект.ПолучитьОбъект();
		ИзмененныйДокумент.ЦППК_Изменен = Ложь;
		ИзмененныйДокумент.Записать();
	КонецЦикла;
	...
	Запрос.Текст = 
		...
		|	И НЕ ЦППК_ИзмененныеДокументы.ИзменяемыйДокумент В (&ИзменяемыйДокумент)";
	
	Запрос.УстановитьПараметр("ИзменяемыйДокумент", СписокИзмененныхДокументов);
	Запрос.УстановитьПараметр("Документ", ТекущийОбъект);
	...
	Пока Выборка.Следующий() Цикл
	    ...
	    НаборЗаписей.Отбор.ИзменяемыйДокумент.Установить(Выборка.ИзменяемыйДокумент);
		НаборЗаписей.Записать();
	КонецЦикла;
