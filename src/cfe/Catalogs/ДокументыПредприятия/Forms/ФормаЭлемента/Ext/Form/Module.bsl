...
#Вставка
	...
	// Котляров В.В. 07.10.2025  САНФ-036848
	Элементы.УчетнаяИнформация.Видимость = Объект.ВидДокумента.ЦППК_ВидимостьВкладкиУчетнаяИнформация;
...
&НаСервереБезКонтекста
Функция УинДопа(ЗНАЧ ИмяДляФормул)
	
	ДополнительныйРеквизит = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту("ИдентификаторДляФормул", ИмяДляФормул);
	Возврат СтрЗаменить(ВРег(Строка(ДополнительныйРеквизит.УникальныйИдентификатор())), "-", "x");
...
Функция ИмяДопРеквизитаНаФорме(ЗНАЧ ИмяДляФормул) 
	...
	УинДопа =  УинДопа(ИмяДляФормул);

	Для каждого ЭлементФормы Из Элементы Цикл
		Если Лев(ЭлементФормы.Имя, 31) = "ДополнительныйРеквизитЗначение_" И Найти(ЭлементФормы.Имя, УинДопа) > 0 И СтрДлина(ЭлементФормы.Имя) <= 104 Тогда
			Возврат ЭлементФормы.Имя;
		КонецЕсли;
	КонецЦикла;
