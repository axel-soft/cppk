...
Процедура ПоказатьВноситИзменения()
	...
	ЭтотВидМожетВноситьИзменения = Делопроизводство.ЭтотВидДокументаМожноИзменить(Объект.ВидДокумента);
	Если Не ВидДокументаКэш.ЦППК_УчитыватьВноситИзменения Тогда
		ЭтотВидМожетВноситьИзменения = Ложь;
	КонецЕсли;
	//Если ПолучитьФункциональнуюОпцию("ЦППК_УчитыватьВноситИзменения") Тогда
...
Процедура ЦППК_ПолеНадписиВноситИзменения_Нажатие(Элемент)
	...
	ВидыИзмененныхДокументов = Делопроизводство.ПолучитьВидыИзмененныхДокументов(Объект.ВидДокумента);
	//
	...
	Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияВводИзмененныхДокументов", ЭтаФорма);
...
Процедура ЦППК_ПрочитатьСвязи(ТипыСвязей)
	...
	Если ТипыСвязей = Неопределено Или ТипыСвязей.Найти(Справочники.ТипыСвязей.ЦППК_ИзмененДокументом) <> Неопределено Тогда 
		...
		ЭтаФорма.ЦППК_СписокИзмененныхДокументов.Очистить();
		СвязиОбъектов.ЦППК_ПрочитатьИзмененные(Объект.Ссылка, Объект.РегистрационныйНомер, ЭтаФорма.ЦППК_СписокИзмененныхДокументов);
		...
		Если ЭтаФорма.ЦППК_СписокИзмененныхДокументов.Количество() <> 0 Тогда
			Элементы.ЦППК_ПолеНадписиВноситИзменения.Заголовок = СтрШаблон(
				НСтр("ru = 'Изменяет (%1)'"), ЭтаФорма.ЦППК_СписокИзмененныхДокументов.Количество());
		Иначе
			Элементы.ЦППК_ПолеНадписиВноситИзменения.Заголовок = НСтр("ru = 'Изменяет'");
		КонецЕсли;	
...
#Вставка
	СвязиОбъектов.ЦППК_УстановитьИзменение(ТекущийОбъект.Ссылка, 
	...
	//		СвязиОбъектов.УстановитьРеквизитыПриДобавленииСвязи(СтрокаДокумент.Документ,,
	//		Справочники.ТипыСвязей.ЦППК_ИзмененДокументом, ДокументЗарегистрированАргумент,
	//		СтрокаДокумент.ДатаОтмены);
