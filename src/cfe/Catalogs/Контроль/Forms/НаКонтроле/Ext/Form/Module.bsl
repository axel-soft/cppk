...
Процедура РекурсивноеЗаполнениеСвязейДокументовИФайлов(СтрокаНачальныйВладелец, ДокументыУчаствующиеВСвязях, УровеньРекурсии)
	...
	|		ПО СвязиОбъектов.СвязанныйОбъект = Файлы.ВладелецФайла И Файлы.ПометкаУдаления = ЛОЖЬ
	|ГДЕ
...
#Вставка
	...
	|						ТОГДА ВЫРАЗИТЬ(ЕСТЬNULL(ВЫРАЗИТЬ(КонтрольИсполнители.Источник КАК Документ.ДействиеЗадачи).РезультатДействияТекстом, "") КАК Строка)
	|					КОГДА КонтрольИсполнители.Источник ССЫЛКА Задача.ЗадачаИсполнителя
	|						ТОГДА ВЫРАЗИТЬ(ЕСТЬNULL(ВЫРАЗИТЬ(КонтрольИсполнители.Источник КАК Задача.ЗадачаИсполнителя).РезультатВыполнения, "") КАК СТРОКА)
