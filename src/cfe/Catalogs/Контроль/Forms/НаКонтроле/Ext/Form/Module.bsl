...
Процедура ОбновитьДеревоПриложенияНаСервере(ДействиеИсполнения) //+ЦППК САНФ-023289, 02.02.2023
	...
	Если ЗначениеЗаполнено(ДокументПредприятия) Тогда
		...
		РекурсивноеЗаполнениеСвязейДокументовИФайлов(СтрокаВладелец, ДокументыУчаствующиеВСвязях);
...
&НаСервереБезКонтекста
Процедура РекурсивноеЗаполнениеСвязейДокументовИФайлов(СтрокаНачальныйВладелец, ДокументыУчаствующиеВСвязях)
	...
	Запрос.Текст = 
		...
		|		ПО СвязиОбъектов.Объект = Файлы.ВладелецФайла
		|ГДЕ
	...
	Пока ВыборкаСвязанныйОбъект.Следующий() Цикл
		Если ДокументыУчаствующиеВСвязях.Найти(ВыборкаСвязанныйОбъект.СвязанныйОбъект) = Неопределено Тогда
			СтрокаВладелец = СтрокаНачальныйВладелец.Строки.Добавить();
			СтрокаВладелец.Ссылка = ВыборкаСвязанныйОбъект.СвязанныйОбъект;
			СтрокаВладелец.ТипФайла = ЗаполнитьКартинкуВСтрокуДерева(8);
			...
			Пока Выборка.Следующий() Цикл
				Если ЗначениеЗаполнено(Выборка.Ссылка) Тогда
					СтрокаФайл = СтрокаВладелец.Строки.Добавить();
					СтрокаФайл.Ссылка = Выборка.Ссылка;
					СтрокаФайл.ТипФайла = ЗаполнитьКартинкуВСтрокуДерева(Выборка.ТипФайла);
				КонецЕсли;
			КонецЦикла;
			Если ДокументыУчаствующиеВСвязях.Найти(СтрокаВладелец.Ссылка) = Неопределено Тогда
				ДокументыУчаствующиеВСвязях.Добавить(СтрокаВладелец.Ссылка);
				РекурсивноеЗаполнениеСвязейДокументовИФайлов(СтрокаВладелец, ДокументыУчаствующиеВСвязях);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
