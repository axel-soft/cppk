...
#Вставка
	Если Ссылка.Публикация = Перечисления.ВариантыПубликацииДополнительныхОтчетовИОбработок.РежимОтладки Тогда
		Результат = Менеджер.Создать("/vs-sed-new-files-prod/Kotlyarov/ОтладкаВнешних/"+Ссылка.ИмяФайла, БезопасныйРежим,
										ОбщегоНазначения.ОписаниеЗащитыБезПредупреждений()); //Результат можно при желании проверить.
