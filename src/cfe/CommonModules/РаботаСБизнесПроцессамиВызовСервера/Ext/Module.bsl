...
#КонецВставки
	...
	Пока ВыборкаЗадачКДобавлению.Следующий() Цикл
        ...
        ЦППК_СтруктураИдентификаторов = Новый Структура;
		//--AxelSoft Шарапова 20.05.2025 САНФ-033767
		#КонецВставки 
		...
		#КонецВставки 
			...
			#Вставка 
			ЦППК_СтруктураИдентификаторов.Вставить("ИдентификаторИсполнителя", ВыборкаЗадачДетальныеЗаписи.ИдентификаторИсполнителя);
			ЦППК_СтруктураИдентификаторов.Вставить("СтрокаЗадача", СтрокаЗадача);
			ЦППК_МассивЗадач.Добавить(ЦППК_СтруктураИдентификаторов);
			#КонецВставки 
		...
		ЦППК_УпорядочитьСтрокиПроцесса(ЦППК_МассивЗадач, МетаданныеПроцесса, ЦППК_БизнесПроцесс, СтрокаПроцесс, Список);
		#КонецВставки
...
Процедура ЦППК_УпорядочитьСтрокиПроцесса(ЦППК_МассивЗадач, МетаданныеПроцесса, ЦППК_БизнесПроцесс, СтрокаПроцесс, Список)
	...
	Если ЦППК_МассивЗадач.Количество() > 1 Тогда
		...
		Для каждого ДетальныеЗаписи Из ЦППК_МассивЗадач Цикл
			...
			Если МетаданныеПроцесса.ТабличныеЧасти.Найти("Исполнители") <> Неопределено Тогда
				ЦППК_НайденныеСтрокиТЧ = ЦППК_БизнесПроцесс.Исполнители.НайтиСтроки(Новый Структура("ИдентификаторИсполнителя", ДетальныеЗаписи.ИдентификаторИсполнителя));
			...
			Если МетаданныеПроцесса.ТабличныеЧасти.Найти("Участники") <> Неопределено Тогда
				ЦППК_НайденныеСтрокиТЧ = ЦППК_БизнесПроцесс.Участники.НайтиСтроки(Новый Структура("Идентификатор", ДетальныеЗаписи.ИдентификаторИсполнителя));
				Если ЦППК_НайденныеСтрокиТЧ.Количество() > 0 Тогда
			...
			Если МетаданныеПроцесса.ТабличныеЧасти.Найти("ИсполнителиИсполнения") <> Неопределено Тогда
				ЦППК_НайденныеСтрокиТЧ = ЦППК_БизнесПроцесс.ИсполнителиИсполнения.НайтиСтроки(Новый Структура("ИдентификаторИсполнителя", ДетальныеЗаписи.ИдентификаторИсполнителя));
				Если ЦППК_НайденныеСтрокиТЧ.Количество() > 0 Тогда
			...
			Если МетаданныеПроцесса.ТабличныеЧасти.Найти("ИсполнителиОзнакомления") <> Неопределено Тогда
				ЦППК_НайденныеСтрокиТЧ = ЦППК_БизнесПроцесс.ИсполнителиОзнакомления.НайтиСтроки(Новый Структура("ИдентификаторИсполнителя", ДетальныеЗаписи.ИдентификаторИсполнителя));
				Если ЦППК_НайденныеСтрокиТЧ.Количество() > 0 Тогда
					ИндексСтрокиБП = ЦППК_БизнесПроцесс.ИсполнителиОзнакомления.Индекс(ЦППК_НайденныеСтрокиТЧ[0]);
				КонецЕсли; 
			КонецЕсли; 
			
			Если ЦППК_НайденныеСтрокиТЧ.Количество() > 1 И ИндексСтрокиБП <> Неопределено Тогда
				
				ИндексСтрокиДерева = СтрокаПроцесс.Строки.Индекс(ДетальныеЗаписи.СтрокаЗадача);	
...
Процедура ЦППК_ЗаполнитьЦиклыПроцессов(СтрокаПроцесс, КонтекстЗадач, ЕстьПравоЧтение)
	...
	//ЦППК_ДействияЗадач = КонтекстЗадач.ДанныеДействийЗадач.ВыгрузитьКолонку("ДействиеЗадачи");
	ЦППК_ДействияЗадач = КонтекстЗадач.АктивныеДействияПоЗадачам.ВыгрузитьКолонку("ДействиеЗадачи");
	...
	Запрос.Текст = 
		...
		|	И ИсторияЗадач.Дата <> ИсторияЗадач.ДействиеЗадачи.ДатаВыполнения
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ДействиеЗадачи
		|;
		...
		|	РеестрЗадачПоАвторам.ВРаботе КАК ВРаботе,
		|	РеестрЗадачПоАвторам.Выполнена КАК Выполнена,
		|	РеестрЗадачПоАвторам.ДатаВыполнения КАК ДатаВыполнения,
		...
		|	РеестрЗадачПоАвторам.Срок КАК Срок,
		|	РеестрЗадачПоАвторам.Флаг КАК Флаг
		|ИЗ
		|	втЗадачи КАК втЗадачи
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрЗадачПоАвторам КАК РеестрЗадачПоАвторам
	...
	Запрос.УстановитьПараметр("ДействияЗадач", ЦППК_ДействияЗадач);
	...
	Пока ДанныеДействия.Следующий() Цикл   
		...
		ХарактерРезультата = РаботаСЗадачамиПовтИсп.ХарактерРезультата(История.РезультатДействия);
		СтрокаЗадача.СостояниеКартинка = 5;
