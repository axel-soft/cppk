...
#Вставка
		// Вывод подвала в новом макете печати согласования
		// Котляров В.В. 07.30.2025 САНФ-035395
		ОбластьПодвал = Макет.ПолучитьОбласть("Подвал");
		УстановитьПривилегированныйРежим(Истина);
		СведенияПользователей = РегистрыСведений.СведенияОПользователях.Получить(Новый Структура("Пользователь", Предмет.Подготовил));
		ОбластьПодвал.Параметры.ДолжностьИсполнителя = РаботаСПользователями.ПолучитьДолжность(Предмет.Подготовил);
		ОбластьПодвал.Параметры.ФИОИсполнителя		 = ФизическиеЛица.ФамилияИнициалыФизЛица(Предмет.Подготовил.Наименование);
		Телефон = ЦППК_ПолучитьТелефонОбъекта(Предмет.Подготовил);
		Если ЗначениеЗаполнено(Телефон) тогда
			ОбластьПодвал.Параметры.Телефон =Телефон;		
		КонецЕсли;
		ТабличныйДокумент.Вывести(ОбластьПодвал);
