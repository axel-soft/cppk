...
Функция ПреобразоватьРимскийЧислоВЧисло(Индекс) Экспорт
    // Котляров 2025,07,09 изменено САНФ-035027
	Разряд = Индекс; //СтрСоединить(СтрРазделить(Индекс, ".-, "),"");
	...
	Если ЗначениеЗаполнено(Результат) Тогда
		Возврат Результат;
	Иначе
		Возврат Число(Разряд); //Число(Разряд)*10;
	КонецЕсли;  
	
КонецФункции

Функция Рассчитать_ЦППК_РазрядПорядка(Индекс) Экспорт

	// Котляров 2025,07,08 САНФ-035027
	Индекс = Стрзаменить(Индекс, "-", "");
	МассивЧастей = СтрРазделить(Индекс, ".,");
	
	ОбразцовыйМассив = Новый Массив(4);
	Для ин=0 По 3 Цикл
		ОбразцовыйМассив[ин] = 0;		
	КонецЦикла;
	Для ин=0 По МассивЧастей.Количество()-1 Цикл
		Если Ин > 3 Тогда Прервать; КонецЕсли;
		Если МассивЧастей[ин] = "" Тогда
		    ОбразцовыйМассив[ин] = 0;
		Иначе
			ОбразцовыйМассив[ин] = МассивЧастей[ин];
		КонецЕсли;
	КонецЦикла;
	
	Для ин=0 По 3 Цикл
		ОбразцовыйМассив[ин] = ЦППК_ОбщийМодульКлиентСервер.ПреобразоватьРимскийЧислоВЧисло(ОбразцовыйМассив[ин]);		
	КонецЦикла;
	
	
	ЦППК_РазрядПорядка = ОбразцовыйМассив[0]*1000000 
	                   + ОбразцовыйМассив[1]*10000 
					   + ОбразцовыйМассив[2]*100 
					   + ОбразцовыйМассив[3]*1   ;
					   
	Возврат ЦППК_РазрядПорядка;
КонецФункции // ()
