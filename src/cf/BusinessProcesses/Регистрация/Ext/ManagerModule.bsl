...
Процедура ЗаполнитьОбрабатывающегоРезультатПоАвтору() Экспорт
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	Регистрация.Ссылка КАК БизнесПроцесс,
		|	Регистрация.Автор
		|ИЗ
		|	БизнесПроцесс.Регистрация КАК Регистрация
		|ГДЕ
		|	Регистрация.ОбрабатывающийРезультат = НЕОПРЕДЕЛЕНО";
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстОшибки = СтрШаблон(
				НСтр("ru = 'При заполнении обрабатывающего результат бизнес-процесса регистрации %1 произошла ошибка:'"),
					Выборка.БизнесПроцесс)
				+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЗаписьЖурналаРегистрации(
				"БизнесПроцессы",
				УровеньЖурналаРегистрации.Ошибка,
				Метаданные.БизнесПроцессы.Регистрация,,
				ТекстОшибки);
