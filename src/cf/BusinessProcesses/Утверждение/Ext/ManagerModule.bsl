...
Процедура ЗаполнитьОбрабатывающегоРезультатПоАвтору() Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
		|	Утверждение.Ссылка КАК БизнесПроцесс,
		|	Утверждение.Автор
		|ИЗ
		|	БизнесПроцесс.Утверждение КАК Утверждение
		|ГДЕ
		|	Утверждение.ОбрабатывающийРезультат = НЕОПРЕДЕЛЕНО";
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если Не ЗначениеЗаполнено(Выборка.Автор) Тогда
				Продолжить;
			КонецЕсли;
			
			БизнесПроцессОбъект = Выборка.БизнесПроцесс.ПолучитьОбъект();
			
			Если ТипЗнч(БизнесПроцессОбъект.Автор) = Тип("СправочникСсылка.Сотрудники") Тогда
				БизнесПроцессОбъект.ОбрабатывающийРезультат = БизнесПроцессОбъект.Автор;
			ИначеЕсли ТипЗнч(БизнесПроцессОбъект.Автор) = Тип("СправочникСсылка.Пользователи") Тогда
				БизнесПроцессОбъект.ОбрабатывающийРезультат =
					Сотрудники.ОсновнойСотрудникПользователя(БизнесПроцессОбъект.Автор);
			КонецЕсли;
		...
		Исключение
			...
			ТекстОшибки = СтрШаблон(
				НСтр("ru = 'При заполнении обрабатывающего результат бизнес-процесса утверждения %1 произошла ошибка:'"),
					Выборка.БизнесПроцесс)
				+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЗаписьЖурналаРегистрации(
				"БизнесПроцессы",
				УровеньЖурналаРегистрации.Ошибка,
				Метаданные.БизнесПроцессы.Утверждение,,
				ТекстОшибки);
