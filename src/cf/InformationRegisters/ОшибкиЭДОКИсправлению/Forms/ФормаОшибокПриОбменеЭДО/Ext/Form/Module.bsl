...
Процедура ПоставитьВОчередьПриема(Команда)
	...
	Если МассивДокументов.Количество() <= 0 Тогда
		ПоказатьПредупреждение(,
			НСтр("ru = 'Среди выделенных строк все документы ожидают автоматического создания.'"), ,
			НСтр("ru = 'Нет доступных документов'"));
		Возврат;
	КонецЕсли;
	...
	Если МассивДокументов.Количество() < ВсегоВыделено Тогда
		...
		ТекстВопроса = СтрШаблон(
			НСтр("ru = 'Из %1 только %2 автоматического создания.'")
				+ Символы.ПС
			...
			СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
				...
				ВсегоВыделено),
			СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
				НСтр("ru = ';%1 не ожидает;;%1 не ожидают;%1 не ожидают;%1 не ожидают'"),
				МассивДокументов.Количество()),
			?(МассивДокументов.Количество() > 1, НСтр("ru = 'их'"), НСтр("ru = 'его'")),
			СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
				НСтр("ru = ';%1 документ;;%1 документа;%1 документов;%1 документов'"),
		...
		ПоказатьВопрос(
			...
			ТекстВопроса,
			РежимДиалогаВопрос.ДаНет, , ,
			НСтр("ru = 'Не все документы доступны'"));
...
Процедура ПоставитьВОчередьПриемаОшибочныеДокументы(МассивДокументов)
	...
	ОбменСКонтрагентамиДОСлужебныйКлиент.ПоставитьДокументыЭДОКАвтоматическомуОтражению(МассивДокументов);
...
Процедура ЗакрытьПринудительноЭД(Команда)
	...
	Если МассивДокументов.Количество() <= 0 Тогда
		ПоказатьПредупреждение(,
			НСтр("ru = 'Среди выделенных строк все документы ожидают автоматического создания фоновым заданием.'")
			 + Символы.ПС
			 + НСтр("ru = 'Завершить обмен принудительно можно только документы не ожидающие автоматиеского создания.'"), ,
			НСтр("ru = 'Нет документов с нужным состоянием'"));
		Возврат;
	КонецЕсли;
...
Функция МассивВыделенныхИсходящихДокументовКИсправлению()
	...
	Для Каждого ИдентификаторСтроки Из ВыделенныеСтроки Цикл
		...
		Если ДанныеСтроки.СостояниеЭДО =
			...
			И ТипЗнч(ДанныеСтроки.ПредметОшибки) = Тип("СправочникСсылка.ДокументыПредприятия") Тогда
...
Функция МассивВыделенныхВходящихДокументовКИсправлению()
	...
	Для Каждого ИдентификаторСтроки Из ВыделенныеСтроки Цикл
		...
		Если Не ДанныеСтроки.АвтоматическоеСоздание Тогда
