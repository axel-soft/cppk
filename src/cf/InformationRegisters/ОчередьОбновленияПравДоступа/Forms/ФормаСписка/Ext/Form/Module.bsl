...
Процедура ОбновитьПорцию(Команда)
	...
	ОбновитьПорциюСервер(10, Ложь); // Без отбора по количеству попыток.
	
КонецПроцедуры

&НаСервере
Функция ОбновитьПорциюСервер(РазмерПорции = 10, УчитыватьЛимитПопыток = Истина)
	...
	Попытка
		...
		ОбработанныхВсего = РегистрыСведений.ОчередьОбновленияПравДоступа.ОбработатьПорцию(
			РазмерПорции, УчитыватьЛимитПопыток);
