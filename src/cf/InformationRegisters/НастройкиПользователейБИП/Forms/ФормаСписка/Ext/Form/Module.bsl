...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтотОбъект.РольДоступнаПолныеПрава = Пользователи.ЭтоПолноправныйПользователь(Неопределено, Ложь, Ложь);
...
Функция ПолучитьПредставлениеНастройкиПросмотраНовостей(Пользователь)
	...
	Если Запись.Выбран() Тогда
		...
		Если ТипЗнч(ЗначениеНастроек) = ТипСтруктура Тогда
			Результат = ОбщегоНазначенияДокументооборотКлиентСервер.ПредставлениеЗначения(
				ЗначениеНастроек,
				": ",
				Символы.ПС);
		Иначе
