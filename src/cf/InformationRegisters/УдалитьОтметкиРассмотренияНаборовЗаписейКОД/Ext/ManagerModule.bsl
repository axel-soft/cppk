...
Функция ПереходНаНовуюВерсиюКоличествоОбъектов() Экспорт
	...
	Запрос = Новый Запрос(
		...
		|ИЗ
		|	РегистрСведений.УдалитьОтметкиРассмотренияНаборовЗаписейКОД КАК Таблица");
...
Функция ВыполнитьПереходНаНовуюВерсиюОтложенный(РазмерПорции = 1000) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Запись = РегистрыСведений.ОтметкиРассмотренияНаборовЗаписейКОД.СоздатьМенеджерЗаписи();
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Не удалось перенести данные РС УдалитьОтметкиРассмотренияНаборовЗаписейКОД
							|по причине:
							|%1'",
			...
			ЗаписьЖурналаРегистрации(
				...
				Метаданные.РегистрыСведений.УдалитьОтметкиРассмотренияНаборовЗаписейКОД, ,
				ТекстСообщения);
		КонецПопытки;
	КонецЦикла;
