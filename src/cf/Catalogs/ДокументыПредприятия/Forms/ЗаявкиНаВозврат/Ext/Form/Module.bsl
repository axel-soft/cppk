...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Добавим в доступные виды документов только заявки на возврат.
	Для каждого ВидДокумента Из ВидыДокументовЗаявкиНаВозврат() Цикл
		ДоступныеВидыДокументов.Добавить(ВидДокумента);
	КонецЦикла;
	...
	Если ДоступныеВидыДокументов.Количество() <> 0 Тогда
		...
		УчитыватьСрокДействия = Ложь; ВестиУчетПоОрганизациям = Ложь; ВестиУчетПоКонтрагентам = Ложь; 
...
Процедура ПриОткрытии(Отказ)
	...
	Элементы.ОтборВажность.Видимость = Ложь; 

	ВидДокумента = ЦППК_ЯвляетсяЗаявкойНаВозврат();
	РаботаСоСпискамиДокументовКлиентСервер.УстановитьОтборВидДокумента(Список, ВидДокумента, Тематика, ДоступныеВидыДокументов);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ЗаписьКонтроля" Тогда
		Если ЗначениеЗаполнено(Параметр.Предмет)
			И ТипЗнч(Параметр.Предмет) = Тип("СправочникСсылка.ДокументыПредприятия") Тогда 
	...
	Если ПоказыватьОбластьОбзора Тогда
		...
		Если ИмяСобытия = "ИзмененыСвязиДокумента" И Параметр.Документ = ТекущийДокумент Тогда
			ОбзорСпискаДокументовКлиент.ОбновитьСтрокуСвязи(ЭтаФорма);
		КонецЕсли;
...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	МК_НастроитьГруппуПросмотраДляМаленькихЭкрановМК(Элементы.ГруппаВидыДокументовМК,
		Элементы.МК_ОчиститьГруппировкуПоВидам, ТекущийВидДокумента);
	...
	МК_НастроитьГруппуПросмотраДляМаленькихЭкрановМК(Элементы.ГруппаВопросыДеятельностиМК,
		Элементы.МК_ОчиститьГруппировкуПоВопросамДеятельности, ТекущийВопросДеятельности);
	
	ТекущийКонтрагент = Настройки["ТекущийКонтрагент"];
	Элементы.Контрагенты.ТекущаяСтрока = ТекущийКонтрагент;
	МК_НастроитьГруппуПросмотраДляМаленькихЭкрановМК(Элементы.ГруппаКонтрагентыМК,
		Элементы.МК_ОчиститьГруппировкуПоКонтрагенту, ТекущийКонтрагент);
	
	ТекущийПроект = Настройки["ТекущийПроект"]; 
	Элементы.Проекты.ТекущаяСтрока = ТекущийПроект;
	МК_НастроитьГруппуПросмотраДляМаленькихЭкрановМК(Элементы.ГруппаПроектыМК,
		Элементы.МК_ОчиститьГруппировкуПоПроекту, ТекущийПроект);
	
	Если ВидПросмотра =  Перечисления.ВидыПросмотраСпискаОбъектов.ПоПапкам Тогда
	
		МК_НастроитьГруппуПросмотраДляМаленькихЭкрановМК(Элементы.ГруппаПапкиМК,
			Элементы.МК_ОчиститьГруппировкуПоПапкам, ТекущаяПапка);
	
	ИначеЕсли ВидПросмотра = Перечисления.ВидыПросмотраСпискаОбъектов.ПоКатегориям Тогда
		
		МК_НастроитьГруппуПросмотраДляМаленькихЭкрановМК(Элементы.ГруппаКатегорииМК,
			Элементы.МК_ОчиститьГруппировкуПоКатегориям, ТекущаяКатегория);
	...
	ОбщегоНазначенияДокументооборотКлиентСервер.ПоказатьСкрытьКнопкуОчисткиОтбора(Элементы.ВидИТематикаТекст, ВидИТематикаТекст);
	ОбщегоНазначенияДокументооборотКлиентСервер.ПоказатьСкрытьКнопкуОчисткиОтбора(Элементы.ОтборПрочитанность, Прочитанность);
	ОбщегоНазначенияДокументооборотКлиентСервер.ПоказатьСкрытьКнопкуОчисткиОтбора(Элементы.ОтборВажность, Важность);
...
Процедура ПапкиПриАктивизацииСтроки(Элемент)
	...
	Если ВидПросмотра = ПредопределенноеЗначение("Перечисление.ВидыПросмотраСпискаОбъектов.ПоПапкам")
		...
		МК_НастроитьГруппуПросмотраДляМаленькихЭкрановМК(Элементы.ГруппаПапкиМК,
			Элементы.МК_ОчиститьГруппировкуПоПапкам,
			ТекущаяПапка);
...
Процедура ПапкиПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	Если Не ЗначениеЗаполнено(Строка) Тогда
		ЭтоПереносПапок = ТипЗнч(ЗначениеПеретаскивания) = Тип("Массив") 
			...
			И ТипЗнч(ЗначениеПеретаскивания[0]) = Тип("СправочникСсылка.ПапкиДокументов");
		...
		Если ИзменитьПапкуДокументов(ЗначениеПеретаскивания, Строка) = Истина Тогда
			...
			Если ЗначениеПеретаскивания.Количество() = 1 Тогда
				ПолноеОписание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Документ ""%1"" перенесен в папку ""%2""'"), ЗначениеПеретаскивания[0], Строка);
				
				ПоказатьОповещениеПользователя(
					НСтр("ru = 'Документ перенесен.'"),
					ПолучитьНавигационнуюСсылку(ЗначениеПеретаскивания[0]),
					ПолноеОписание,
					БиблиотекаКартинок.Информация32);
			Иначе
				ПолноеОписание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Документы (%1 шт.) перенесены в папку ""%2""'"), ЗначениеПеретаскивания.Количество(), Строка);
				
				ПоказатьОповещениеПользователя(
					НСтр("ru = 'Документы перенесены.'"),
					,
					ПолноеОписание,
					БиблиотекаКартинок.Информация32);
			КонецЕсли;
...
Процедура ВидыДокументовПриАктивизацииСтроки(Элемент)
	...
	Если ВидПросмотра = ПредопределенноеЗначение("Перечисление.ВидыПросмотраСпискаОбъектов.ПоВидамДокументов")
		...
		МК_НастроитьГруппуПросмотраДляМаленькихЭкрановМК(Элементы.ГруппаВидыДокументовМК,
			Элементы.МК_ОчиститьГруппировкуПоВидам,
			ТекущийВидДокумента);
...
Процедура ВопросыДеятельностиПриАктивизацииСтроки(Элемент)
	...
	Если ВидПросмотра = ПредопределенноеЗначение("Перечисление.ВидыПросмотраСпискаОбъектов.ПоВопросамДеятельности")
		...
		МК_НастроитьГруппуПросмотраДляМаленькихЭкрановМК(Элементы.ГруппаВопросыДеятельностиМК,
			Элементы.МК_ОчиститьГруппировкуПоВопросамДеятельности,
			ТекущийВопросДеятельности);
...
Процедура КонтрагентыПриАктивизацииСтроки(Элемент)
	...
	Если ВидПросмотра = ПредопределенноеЗначение("Перечисление.ВидыПросмотраСпискаОбъектов.ПоКонтрагентам")
		...
		МК_НастроитьГруппуПросмотраДляМаленькихЭкрановМК(Элементы.ГруппаКонтрагентыМК,
			Элементы.МК_ОчиститьГруппировкуПоКонтрагенту,
			ТекущийКонтрагент);
...
Процедура СписокНоменклатураДелПриАктивизацииСтроки(Элемент)
	...
	Если ВидПросмотра = ПредопределенноеЗначение("Перечисление.ВидыПросмотраСпискаОбъектов.ПоНоменклатуреДел")
		...
		МК_НастроитьГруппуПросмотраДляМаленькихЭкрановМК(Элементы.ГруппаНоменклатураДелМК,
			Элементы.МК_ОчиститьГруппировкуПоНоменклатуреДел,
			ТекущаяНоменклатураДел);
...
Процедура ДелаТомаПриАктивизацииСтроки(Элемент)
	...
	Если ВидПросмотра = ПредопределенноеЗначение("Перечисление.ВидыПросмотраСпискаОбъектов.ПоДеламТомам")
		...
		МК_НастроитьГруппуПросмотраДляМаленькихЭкрановМК(Элементы.ГруппаДелаТомаМК,
			Элементы.МК_ОчиститьГруппировкуПоДелам,
			ТекущееДелоТом);
...
Процедура ОтборПрочитанностьПриИзменении(Элемент)
	...
	РаботаСоСпискамиДокументовКлиентСервер.УстановитьОтборСписка(Список, ПараметрыОтбора);
	ОбщегоНазначенияДокументооборотКлиентСервер.ПоказатьСкрытьКнопкуОчисткиОтбора(Элемент, Прочитанность);
	...
	Если Не Копирование Тогда
		Попытка
			...
			РаботаСФайламиКлиент.ДобавитьФайл(Неопределено, ВладелецФайла, 
				...
				,,,,,
				РольФайла, , ДополнительныеПараметры);
		Исключение
...
Процедура ФайлыПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
		...
		Если ВВидеЭтаРольТолькоОдинФайл(ВидДокументаКэш.РолиФайлов, РольФайла) Тогда
			...
			Иначе
				...
				Если ДрагДропНесколькоФайлов Тогда
					...
					ПоказатьПредупреждение(, 
						СтрШаблон(НСтр("ru = 'В роль ""%1"" с признаком ""Только один файл"" можно добавить только один файл.'"), РольФайла));
	...
	Если ТипЗнч(НедоступныеПоля) = Тип("Структура") 
		И НедоступныеПоля.Свойство("КонтекстноеМенюФайлыРедактировать")
		И ТекущийЧислоРолейФайлов <> 0
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	РаботаСоСпискамиДокументовКлиент.ОбработкаВыбора(ЭтотОбъект, Элементы.Список, Поле, СтандартнаяОбработка);
...
Процедура СписокПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки) 
	...
	Для Каждого КлючИЗначение Из Строки Цикл 
		...
		ОписаниеДокумента = Новый Структура("Ссылка, ВидДокумента",
			ДанныеСтроки.Ссылка, ДанныеСтроки.ВидДокумента);
	...
	Для Каждого КлючИЗначение Из Строки Цикл
		...
		Если ДанныеСтроки.Свойство("Подготовил")
			И ЗначениеЗаполнено(ДанныеСтроки.Подготовил)
	...
	ИменаРеквизитовВидовДокументов =
		"ОбязателенФайлОригинала, КонтролироватьНаличиеОригинала, ЯвляетсяКомплектомДокументов";
	...
	Для Каждого КлючИЗначение Из Строки Цикл
		...
		Если ДанныеСтроки.Свойство("ИндексКартинкиФайлы") Тогда
			...
			Если РеквизитыВидаДокумента.КонтролироватьНаличиеОригинала
				И Не ДанныеСтроки.ОригиналПолучен
				И Не ДанныеСтроки.РегистрационныйНомер = "" Тогда
				
				ИндексКартинкиФайлы = 4;
				
			ИначеЕсли Не ВестиУчетСканКопийОригиналовДокументов
				Или Не РеквизитыВидаДокумента.ОбязателенФайлОригинала
				Или ДанныеСтроки.СодержитОригинал
				Или ДанныеСтроки.РегистрационныйНомер = "" Тогда
				
				Если Не ДанныеСтроки.ЕстьФайлы Тогда
					
					ИндексКартинкиФайлы = 0;
					
				ИначеЕсли ДанныеСтроки.СодержитОригинал
			...
			Иначе
				...
				Если Не ДанныеСтроки.ЕстьФайлы
					Или (РеквизитыВидаДокумента.КонтролироватьНаличиеОригинала
		...
		Если ДанныеСтроки.Свойство("Подготовил")
			И ДанныеСтроки.Свойство("ПодготовилНедействителен") Тогда
			
			Если ЗначениеЗаполнено(ДанныеСтроки.Подготовил)
				И ТипЗнч(ДанныеСтроки.Подготовил) = Тип("СправочникСсылка.Сотрудники") Тогда
			...
			ИначеЕсли ЗначениеЗаполнено(ДанныеСтроки.Подготовил)
				И ТипЗнч(ДанныеСтроки.Подготовил) = Тип("СправочникСсылка.Пользователи") Тогда
...
&НаКлиенте
Процедура СписокСортироватьПоНаименованиюМК(Команда)
...
&НаКлиенте
Процедура СписокСортироватьПоПодготовилМК(Команда)
	
	ПереключитьСортировку(СписокСортироватьПо, СписокНаправлениеСортировки, "Подготовил");
	СортироватьСписокПоКолонке("Подготовил", ЭтотОбъект);
	
КонецПроцедуры

&НаКлиенте
Процедура СписокСортироватьПоРегНомеруМК(Команда)
	
	ПереключитьСортировку(СписокСортироватьПо, СписокНаправлениеСортировки, "РегистрационныйНомерИДата");
	СортироватьСписокПоКолонке("РегистрационныйНомерИДата", ЭтотОбъект);
	
КонецПроцедуры

&НаКлиенте
Процедура СписокСортироватьПоДатеМК(Команда)
	
	ПереключитьСортировку(СписокСортироватьПо, СписокНаправлениеСортировки, "ДатаСортировки");
...
Процедура УстановитьВидимостьПоУсловиямИФункционалнымОпциям()
	...
	Элементы.СостояниеКонтроля.Видимость = ПолучитьФункциональнуюОпцию("ИспользоватьКонтрольОбъектов");
	Элементы.ИндексСостоянияДела.Видимость = ПолучитьФункциональнуюОпцию("ИспользоватьНоменклатуруДел")
		И ДелопроизводствоСерверПовтИсп.ИспользуетсяПередачаДелХранения();
	Элементы.СостояниеЭДО.Видимость = ОбщегоНазначенияБЭД.ЗначениеФункциональнойОпции("ИспользоватьОбменЭД");
...
Процедура ОбработатьПеретаскиваниеДокументов(ПараметрыПеретаскивания, ИмяРеквизита, ЗначениеРеквизита, Синоним)
	...
	Если ТипЗнч(ЗначениеПеретаскивания) = Тип("Массив") 
		...
		Если ИзменитьРеквизитДокументов(
			...
			Если ЗначениеПеретаскивания.Количество() = 1 Тогда
				ПолноеОписание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Документ ""%1"" перенесен в ""%2""'"), ЗначениеПеретаскивания[0], ЗначениеРеквизита);
			...
			Иначе
				ПолноеОписание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Документы (%1 шт.) перенесены в ""%2""'"), ЗначениеПеретаскивания.Количество(), ЗначениеРеквизита);
...
Процедура ОбработатьПеретаскиваниеФайлов(ПараметрыПеретаскивания, ИмяРеквизита, ЗначениеРеквизита)
	...
	Если ТипЗнч(ПараметрыПеретаскивания.Значение) = Тип("Файл") 
	   И ПараметрыПеретаскивания.Значение.ЭтоФайл() Тогда
		...
		ОткрытьФорму("Справочник.ДокументыПредприятия.ФормаОбъекта", ПараметрыОткрытияФормы, Элементы.Список);
...
Процедура СоздатьНовыйДокумент(Копирование, ПараметрыФормы = Неопределено)
	...
	Иначе
		...
		Иначе
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Документ можно создавать только по шаблону. Обратитесь к администратору'"));
		КонецЕсли;
...
Процедура ПостроитьДеревоКатегорий() Экспорт 
	...
	Дерево = РаботаСКатегориямиДанных.ПостроитьДеревоКатегорий(Дерево,, Истина, Перечисления.ТипыОбъектов.ДокументыПредприятия);	
...
Процедура ВыборФайла(Файл)
	...
	Если ДанныеФайла.ТребуетсяСоздатьВизуализациюЭП
		...
		#Если Не ВебКлиент И Не МобильныйКлиент Тогда
			...
			РаботаСФайламиКлиент.СоздатьВизуализациюЭПВФайлеDoc(
...
Процедура ОтметитьПризнакОригиналУНовогоФайла(Файл)
	...
	ДоступноТолькоДобавлениеОригинала = ТипЗнч(ДоступныеПоля) = Тип("Структура") 
		...
		Если ДоступноТолькоДобавлениеОригинала 
			Или (ДелопроизводствоКлиентСервер.ЭтоРасширениеСканКопии(НайденнаяСтрока.Расширение) 
...
&НаСервереБезКонтекста
Функция ВидыДокументовЗаявкиНаВозврат()
	...
	|	И ВидыДокументов.ЯвляетсяЗаявкойНаВозврат";
...
&НаКлиенте
Процедура ПоказыватьОписаниеИлиЗадачиПриИзменении(Элемент)
	
	Если ПоказыватьОписаниеИлиЗадачи = "Задачи" Тогда 
...
#Если МобильныйКлиент Тогда
	Если ПоказыватьОписаниеИлиЗадачи = "Файлы" Тогда
		Элементы.Страницы.ТекущаяСтраница = Элементы.ГруппаФайлыМК;
	КонецЕсли;
#КонецЕсли
...
Процедура НастроитьФормуДляМобильногоПриНеобходимости()
	...
	// Настроим команды и командную панель.
	...
	// Настроим список документов.
	Элементы.Список.ВертикальныеЛинии = Ложь;

	// Настроим строку поиска.
	...
	Элементы.ВременныйНомер.Видимость = Ложь;
	...
	// Первая колонка.
	Элементы.Переместить(Элементы.КоличествоВажныхСвязей, Элементы.ГруппаФайлыЗадачи);
	// Вторая колонка.
	Элементы.Переместить(Элементы.СостояниеКонтроля, Элементы.ГруппаЭПКатегории);
	// Третья колонка.
	Элементы.ГруппаВертикальнаяМК.Видимость = Истина;
	...
	Элементы.Переместить(Элементы.Подготовил, Элементы.ГруппаПодготовилМК);
	Элементы.Подготовил.ЦветТекста = WebЦвета.ТемноСерый;
	Элементы.Подготовил.Высота = 1;
	Элементы.Подготовил.АвтоВысотаЯчейки = Ложь;
	
	Элементы.Переместить(Элементы.РегистрационныйНомерИДата, Элементы.ГруппаРегНомерДатаИДатаМК);
	Элементы.РегистрационныйНомерИДата.Высота = 1;
	Элементы.РегистрационныйНомерИДата.АвтоВысотаЯчейки = Ложь;
	
	Элементы.Переместить(Элементы.ДатаСортировки, Элементы.ГруппаРегНомерДатаИДатаМК);
	Элементы.ДатаСортировки.Высота = 1;
	Элементы.ДатаСортировки.АвтоВысотаЯчейки = Ложь;
	
	// Настроим формат Даты.
	ИспользоватьДатуИВремяВСрокахЗадач = ПолучитьФункциональнуюОпцию("ИспользоватьДатуИВремяВСрокахЗадач");
	ФорматДатыДляКолонок = ?(ИспользоватьДатуИВремяВСрокахЗадач, НСтр("ru='ДФ=''dd.MM.yy'''"), "ДЛФ=D");
	Элементы.ДатаСортировки.Формат = ФорматДатыДляКолонок;
	
	// Настроим фильтры.
	Пока Элементы.ГруппаБыстрыеОтборы.ПодчиненныеЭлементы.Количество() > 0 Цикл
		
		ПолеФильтра = Элементы.ГруппаБыстрыеОтборы.ПодчиненныеЭлементы[0];
		
		Элементы.Переместить(ПолеФильтра, Элементы.ГруппаБыстрыеОтборыМК);
		ПолеФильтра.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
				
	КонецЦикла;
	
	Элементы.ГруппаФильтрыИСписокМК.Видимость = Истина;
	Элементы.ГруппаБыстрыеОтборы.Видимость = Ложь;
	Элементы.Переместить(Элементы.Список, Элементы.ГруппаФильтрыИСписокМК);
	...
	// Настроим области просмотра.
	Элементы.СтраницыДеревоРазрезов.Ширина = 20;
	
	// Настроим важность колонок списка для отображения на маленьком экране.
	// Картинки.
	Элементы.Файлы.ВажностьПриОтображении = ВажностьПриОтображении.Низкая;
	Элементы.Задачи.ВажностьПриОтображении = ВажностьПриОтображении.Низкая;
	Элементы.ПодписанЭП.ВажностьПриОтображении = ВажностьПриОтображении.Низкая;
	Элементы.КоличествоВажныхСвязей.ВажностьПриОтображении = ВажностьПриОтображении.Низкая;
	...
	Элементы.ДатаСортировки.ВажностьПриОтображении = ВажностьПриОтображении.Обычная;
...
Процедура ПриИзмененииПараметровЭкранаНаСервере()
	...
	Элементы.ГруппаОтборыИМиникарточка.Видимость = ЭтоМобильныйСБольшимЭкраном И Не ЭтоМобильныйВПортретномРежиме;
	
	Если ЭтоМобильныйСБольшимЭкраном И Не ЭтоМобильныйВПортретномРежиме Тогда
		НастроитьОбластьПредпросмотраМК();
	Иначе
		НастроитьПросмотрПоРазрезамДляМаленькихЭкрановМК();
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура НастроитьОбластьПредпросмотраМК()
	
	// Настроим миникарточку.
	Элементы.ГруппаМиникарточка.ОтображатьЗаголовок = Ложь;
	Элементы.ГруппаМиникарточка.Ширина = 25;
	
	ЭтаФорма.СворачиваниеЭлементовПоВажности = СворачиваниеЭлементовФормыПоВажности.НеИспользовать;
	Элементы.Список.ИспользованиеТекущейСтроки = 
		ИспользованиеТекущейСтрокиТаблицы.ОтображениеВыделенияИВыбор;
		
	Элементы.ГруппаДеревьяИСписок.Группировка = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда;
	// Настроим интервалы.
	Элементы.ГруппаДеревьяИСписок.ГоризонтальныйИнтервал = ИнтервалМеждуЭлементамиФормы.Половинный;
	
	// Добавим переключатель на страницу с файлами.
	Элементы.ГруппаФайлыМК.Видимость = Истина;
	Элементы.Переместить(Элементы.ФайлыДокумента, Элементы.ГруппаФайлыМК);
	Элементы.ПоказыватьОписаниеИлиЗадачи.СписокВыбора.Добавить("Файлы", "Файлы");
...
&НаСервере
Процедура НастроитьПросмотрПоРазрезамДляМаленькихЭкрановМК()
	
	Элементы.ГруппаКатегорииМК.Видимость = Истина;
	Элементы.Переместить(Элементы.ДеревоКатегорий, Элементы.ГруппаКатегорииМК);
	Элементы.Переместить(Элементы.ВыбранныеКатегории, Элементы.ГруппаКатегорииМК);
	
	Элементы.ГруппаПапкиМК.Видимость = Истина;
	Элементы.Переместить(Элементы.Папки, Элементы.ГруппаПапкиМК);
	
	Элементы.ГруппаВидыДокументовМК.Видимость = Истина;
	Элементы.Переместить(Элементы.ВидыДокументов, Элементы.ГруппаВидыДокументовМК);
	
	Элементы.ГруппаВопросыДеятельностиМК.Видимость = Истина;
	Элементы.Переместить(Элементы.ВопросыДеятельности, Элементы.ГруппаВопросыДеятельностиМК);
	
	Элементы.ГруппаКонтрагентыМК.Видимость = Истина;
	Элементы.Переместить(Элементы.Контрагенты, Элементы.ГруппаКонтрагентыМК);
	
	Элементы.ГруппаНоменклатураДелМК.Видимость = Истина;
	Элементы.Переместить(Элементы.СписокНоменклатураДел, Элементы.ГруппаНоменклатураДелМК);
	Элементы.Переместить(Элементы.ГруппаОрганизацияГод, Элементы.ГруппаНоменклатураДелМК);
	
	Элементы.ГруппаДелаТомаМК.Видимость = Истина;
	Элементы.Переместить(Элементы.ДелаТома, Элементы.ГруппаДелаТомаМК);
	Элементы.Переместить(Элементы.ГруппаОрганизацияГодДелТомов, Элементы.ГруппаДелаТомаМК);
	
	Элементы.ГруппаПроектыМК.Видимость = Истина;
	Элементы.Переместить(Элементы.Проекты, Элементы.ГруппаПроектыМК);
	Элементы.Переместить(Элементы.ТолькоМоиПроекты, Элементы.ГруппаПроектыМК);

КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура МК_НастроитьГруппуПросмотраДляМаленькихЭкрановМК(ЭлементГруппаПросмотра,
	ЭлементКомандаОчистки,
	...
	Если МК_ВызовСервера.ЭтоМобильныйСБольшимЭкраном() Тогда
		Возврат;
	КонецЕсли;
	
	Если Не ЭлементГруппаПросмотра.Скрыта() Тогда 
		ЭлементГруппаПросмотра.Скрыть();
	КонецЕсли;
	
	ЕстьГруппировка = ЗначениеЗаполнено(ТекущаяГруппировка);
	ЭлементГруппаПросмотра.Заголовок = ?(ЕстьГруппировка, Строка(ТекущаяГруппировка), ЭлементГруппаПросмотра.Подсказка);
	
	ЭлементКомандаОчистки.Видимость = ЕстьГруппировка;

КонецПроцедуры

&НаКлиенте
Процедура МК_ОчиститьГруппировкуПоВидам(Команда)
	
	Элементы.ВидыДокументов.ТекущаяСтрока = Неопределено;
	ВидыДокументовПриАктивизацииСтроки(Элементы.ВидыДокументов);
	
КонецПроцедуры

&НаКлиенте
Процедура МК_ОчиститьГруппировкуПоВопросамДеятельности(Команда)

	Элементы.ВопросыДеятельности.ТекущаяСтрока = Неопределено;
	ВопросыДеятельностиПриАктивизацииСтроки(Элементы.ВопросыДеятельности);
	
КонецПроцедуры

&НаКлиенте
Процедура МК_ОчиститьГруппировкуПоКонтрагенту(Команда)
	
	Элементы.Контрагенты.ТекущаяСтрока = Неопределено;
	КонтрагентыПриАктивизацииСтроки(Элементы.Контрагенты);
	
КонецПроцедуры

&НаКлиенте
Процедура МК_ОчиститьГруппировкуПоНоменклатуреДел(Команда)

	Элементы.СписокНоменклатураДел.ТекущаяСтрока = Неопределено;
	СписокНоменклатураДелПриАктивизацииСтроки(Элементы.СписокНоменклатураДел);

КонецПроцедуры

&НаКлиенте
Процедура МК_ОчиститьГруппировкуПоПроекту(Команда)
	
	Элементы.Проекты.ТекущаяСтрока = Неопределено;
	ПроектыПриАктивизацииСтроки(Элементы.Проекты);
	
КонецПроцедуры

&НаКлиенте
Процедура МК_ОчиститьГруппировкуПоДелам(Команда)

	Элементы.ДелаТома.ТекущаяСтрока = Неопределено;
	ДелаТомаПриАктивизацииСтроки(Элементы.ДелаТома);

КонецПроцедуры

&НаКлиенте
Процедура МК_ОчиститьГруппировкуПоПапкам(Команда)
...
&НаКлиенте
Процедура МК_ОчиститьГруппировкуПоКатегориям(Команда)
...
Процедура УстановитьПометкуСортировкиМК(Форма)
	...
	Форма.Элементы.СписокСортироватьПоНаименованиюМК.Пометка = Ложь;
	Форма.Элементы.СписокСортироватьПоНаименованиюМК.Заголовок = НСтр("ru = 'Наименование'");
	
	Форма.Элементы.СписокСортироватьПоРегНомеруМК.Пометка = Ложь;
	Форма.Элементы.СписокСортироватьПоРегНомеруМК.Заголовок = НСтр("ru = 'Рег. номер'");
	
	Форма.Элементы.СписокСортироватьПоПодготовилМК.Пометка = Ложь;
	Форма.Элементы.СписокСортироватьПоПодготовилМК.Заголовок = НСтр("ru = 'Подготовил'");
	
	Форма.Элементы.СписокСортироватьПоДатеМК.Пометка = Ложь;
	Форма.Элементы.СписокСортироватьПоДатеМК.Заголовок = НСтр("ru = 'Дата'");
	
	Если Форма.СписокСортироватьПо = "Заголовок" Тогда
		Форма.Элементы.СписокСортироватьПоНаименованиюМК.Пометка = Истина;
		Форма.Элементы.СписокСортироватьПоНаименованиюМК.Заголовок = 
			СтрШаблон("%1 (%2)",
				НСтр("ru = 'Наименование'"),
				Форма.СписокНаправлениеСортировки);
				
	ИначеЕсли Форма.СписокСортироватьПо = "РегистрационныйНомерИДата" Тогда
		Форма.Элементы.СписокСортироватьПоРегНомеруМК.Пометка = Истина;
		Форма.Элементы.СписокСортироватьПоРегНомеруМК.Заголовок = 
			СтрШаблон("%1 (%2)",
				НСтр("ru = 'Рег. номер'"),
				Форма.СписокНаправлениеСортировки);
				
	ИначеЕсли Форма.СписокСортироватьПо = "Подготовил" Тогда
		Форма.Элементы.СписокСортироватьПоПодготовилМК.Пометка = Истина;
		Форма.Элементы.СписокСортироватьПоПодготовилМК.Заголовок = 
			СтрШаблон("%1 (%2)",
				НСтр("ru = 'Подготовил'"),
				Форма.СписокНаправлениеСортировки);
				
	ИначеЕсли Форма.СписокСортироватьПо = "ДатаСортировки" Тогда
		Форма.Элементы.СписокСортироватьПоДатеМК.Пометка = Истина;
		Форма.Элементы.СписокСортироватьПоДатеМК.Заголовок = 
			СтрШаблон("%1 (%2)",
				НСтр("ru = 'Дата'"),
				Форма.СписокНаправлениеСортировки);
...
// Добавлен отбор на форму списка по ФИО билетного кассира  для заявок на возврат
&НаКлиенте
Процедура ОтборПоКассиру_ТТСПриИзменении(Элемент)
	ПараметрыОтбора = Новый Соответствие;
	ПараметрыОтбора.Вставить("ФИОБК_ТТС", ЭтотОбъект["ФИОБК_ТТС"]);
	
	РаботаСоСпискамиДокументовКлиентСервер.УстановитьОтборСписка(Список, ПараметрыОтбора);
	
	ОбщегоНазначенияДокументооборотКлиентСервер.ПоказатьСкрытьКнопкуОчисткиОтбора(Элемент, ЭтотОбъект["ФИОБК_ТТС"]);

КонецПроцедуры

// Отбор по месту рассмотрения
&НаКлиенте
Процедура МестоРассмотрения_ТТСПриИзменении(Элемент)
		
	ПараметрыОтбора = Новый Соответствие;
	ПараметрыОтбора.Вставить("МестоРассмотрения", ЭтотОбъект["МестоРассмотрения_ТТС"]);
	
	РаботаСоСпискамиДокументовКлиентСервер.УстановитьОтборСписка(Список, ПараметрыОтбора);
	
	ОбщегоНазначенияДокументооборотКлиентСервер.ПоказатьСкрытьКнопкуОчисткиОтбора(Элемент, ЭтотОбъект["МестоРассмотрения_ТТС"]);
...
Процедура ОтборКорреспондентПриИзменении(Элемент)
	...
	//ПараметрыОтбора.Вставить("ФИОЗаявителя_ТТС", ЭтотОбъект["ФИОЗаявителя_ТТС"]);
	//РаботаСоСпискамиДокументовКлиентСервер.УстановитьОтборСписка(Список, ПараметрыОтбора);
	//ОбщегоНазначенияДокументооборотКлиентСервер.ПоказатьСкрытьКнопкуОчисткиОтбора(Элемент, ЭтотОбъект["ФИОЗаявителя_ТТС"]);
	
	ПараметрыОтбора.Вставить("ЦППК_ФИОЗаявителя", ЭтотОбъект["ЦППК_ФИОЗаявителя"]);
	
	РаботаСоСпискамиДокументовКлиентСервер.УстановитьОтборСписка(Список, ПараметрыОтбора);
	
	ОбщегоНазначенияДокументооборотКлиентСервер.ПоказатьСкрытьКнопкуОчисткиОтбора(Элемент, ЭтотОбъект["ЦППК_ФИОЗаявителя"]);
	//--AxelSoft Шарапова 11.11.2024 САНФ-029223
...
&НаСервере
Функция ЦППК_ЯвляетсяЗаявкойНаВозврат()
   Заявка = Справочники.ВидыДокументов.НайтиПоНаименованию("Заявление на возврат денежных средств по претензии");
   
   Если Заявка = Неопределено Тогда
      Возврат Справочники.ВидыДокументов.ПустаяСсылка();
   Иначе	
      Возврат Заявка;
   КонецЕсли;
