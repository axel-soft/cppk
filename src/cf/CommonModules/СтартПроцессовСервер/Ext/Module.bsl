...
Процедура ЗаполнитьНастройкиСтартаВФормеПроцесса(Форма) Экспорт
	...
	Иначе
		...
		Форма.НастройкаСтарта = РегистрыСведений.НастройкиОтложеногоСтартаПроцессов.ПолучитьСведенияОЗапускеПроцесса(
			Форма.Объект.Ссылка);
...
Процедура ОбновитьНастройкиСтартаПроцессаИзФормы(Процесс, ПараметрыЗаписи, НастройкаСтарта) Экспорт
	...
	СохраненныеНастройки = 
		РегистрыСведений.НастройкиОтложеногоСтартаПроцессов.ПолучитьСведенияОЗапускеПроцесса(Процесс);
...
Процедура СтартоватьПроцессОтложенно(БизнесПроцесс) Экспорт
	...
	// При добавлении будет заблокирована соответствующая запись настройки в РС НастройкиОтложеногоСтартаПроцессов.
	РегистрыСведений.НастройкиОтложеногоСтартаПроцессов.Добавить(
...
Процедура ОтключитьОтложенныйСтарт(БизнесПроцесс) Экспорт
	...
	// При отключении будет заблокирована соответствующая запись настройки в РС НастройкиОтложеногоСтартаПроцессов.
	РегистрыСведений.НастройкиОтложеногоСтартаПроцессов.ОтключитьОтложенныйСтартПроцесса(Настройка.БизнесПроцесс);
...
Процедура ПроцессПриЗаписи(БизнесПроцесс, Отказ) Экспорт
	...
	НастройкаСтарта = РегистрыСведений.НастройкиОтложеногоСтартаПроцессов.ПолучитьСведенияОЗапускеПроцесса(
		БизнесПроцесс.Ссылка);
...
Процедура СтартОтложенныхПроцессов() Экспорт
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	РегистрСведений.НастройкиОтложеногоСтартаПроцессов КАК ПроцессыДляЗапуска
	...
	Пока Выборка.Следующий() Цикл
		...
		ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.НастройкиОтложеногоСтартаПроцессов");
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		ЭлементБлокировки.УстановитьЗначение("БизнесПроцесс", Выборка.БизнесПроцесс);
		
		НачатьТранзакцию();
		
		// Пытаемся заблокировать процесс и запись в регистре НастройкиОтложеногоСтартаПроцессов.
		...
		Попытка
			...
			РегистрыСведений.НастройкиОтложеногоСтартаПроцессов.ЗарегистрироватьСтартПроцесса(Выборка.БизнесПроцесс);
		...
		Исключение
			...
			Если Выборка.КоличествоПопытокОбработки < 3 Тогда
				РегистрыСведений.НастройкиОтложеногоСтартаПроцессов.ЗарегистрироватьПопыткуСтартаПроцесса(Выборка.БизнесПроцесс);
			Иначе
				...
				РегистрыСведений.НастройкиОтложеногоСтартаПроцессов.ЗарегистрироватьОтменуСтарта(
