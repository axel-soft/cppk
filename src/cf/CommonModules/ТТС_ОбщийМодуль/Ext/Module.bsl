	...
	Если СтруктураФайла.Свойство("SEDID") Тогда  
		...
		Если ЗначениеЗаполнено(НайденныйДокумент) Тогда
			...
			Для каждого СтрокаФайл Из СтруктураФайла.Файлы Цикл
				...
				// ++AxelSoft Котляров 14.08.2025 САНФ-035736 Ошибка. Пустое содержимое файла.
				Если СтрокаФайл.ДанныеФайла = Неопределено Тогда
					
					ЗаписьЖурналаРегистрации(НСтр("ru = 'Обмен ОМиУ'", ОбщегоНазначения.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,,,
					"(Обмен ОМиУ) Файл не добавлен: отсутствие двоичных данных."
		...
		Пока Чтение.Прочитать() Цикл
			...
			ИначеЕсли Чтение.ТипУзла = ТипУзлаXML.Текст Тогда
				...
				ИначеЕсли ИмяУзла = "BinaryAttach" Тогда
					...
					// ++AxelSoft Котляров 14.08.2025 САНФ-035736 Ошибка. Пустое содержимое файла.
					Если НЕ ЗначениеЗаполнено(СокрЛП(Чтение.Значение)) Тогда
						СтрокаТЗ.ДанныеФайла = Неопределено;				
					Иначе	
						Попытка
