...
Процедура ПередЗаписьюБизнесПроцессаПередЗаписью(Источник, Отказ) Экспорт
	...
	Если РаботаСБизнесПроцессамиХолдинг.ДобавитьЗаданиеОбновитьПроцессБезДействия(Источник) Тогда
		Источник.ОбменДанными.Загрузка = Истина;
		Источник.ДополнительныеСвойства.Вставить("ОтключитьОтметкиВремени", Истина);
		Возврат;
	КонецЕсли;	
...
Процедура БизнесПроцессПриЗаписиПриЗаписи(Источник, Отказ) Экспорт
	...
	Если ОбычнаяЗапись Тогда
		...
		Если ПроцессПомеченНаУдаление Тогда
			РегистрыСведений.НастройкиОтложеногоСтартаПроцессов.Удалить(Источник.Ссылка);
		КонецЕсли;
...
Процедура ОбновитьДоступностьЭлементовФормыПоСостояниюПроцесса(
	...
	ПроцессЕстьВОчередиЗаданий = РаботаСБизнесПроцессамиХолдинг.ПроцессИлиЕгоЗадачиЕстьВОчередиЗаданий(
		Форма.Объект.Ссылка);
