...
Процедура ПрерватьБизнесПроцессИзФормыОбъектаОкончание(Форма, Результат) Экспорт
	...
	ОбъектСсылка = Форма.Объект.Ссылка;
	Если РаботаСБизнесПроцессамиХолдингКлиент.ОбработатьПрерываниеПроцессаИзКарточки(Форма, Неопределено,
		Результат.ПричинаПрерывания) Тогда
		ПротоколированиеРаботыСотрудников.ЗаписатьПрерываниеБизнесПроцесса(ОбъектСсылка);
		ПоказатьОповещениеПользователя(
		НСтр("ru = 'Процесс добавлен в очередь заданий и будет прерван'"), ПолучитьНавигационнуюСсылку(ОбъектСсылка),
			Строка(ОбъектСсылка),
...
Процедура ИзменитьСостояниеБизнесПроцессаИзФормы(Форма, НовоеСостояние, ТекстСообщения)
	...
	Если НовоеСостояние = ПредопределенноеЗначение("Перечисление.СостоянияБизнесПроцессов.Остановлен") Тогда
		Если РаботаСБизнесПроцессамиХолдингКлиент.ОбработатьОстановкуПроцессаИзКарточки(Форма)
			Тогда
			ПоказатьОповещениеПользователя(
				ТекстСообщения, ПолучитьНавигационнуюСсылку(Форма.Объект.Ссылка), Строка(Форма.Объект.Ссылка),
				БиблиотекаКартинок.Информация32);
			ОповеститьОбИзменении(Форма.Объект.Ссылка);
			Возврат;
		КонецЕсли;
	ИначеЕсли НовоеСостояние = ПредопределенноеЗначение("Перечисление.СостоянияБизнесПроцессов.Активен") Тогда
		Если РаботаСБизнесПроцессамиХолдингКлиент.ОбработатьПродолжениеПроцессаИзКарточки(Форма)
			Тогда
			ПоказатьОповещениеПользователя(
				ТекстСообщения, ПолучитьНавигационнуюСсылку(Форма.Объект.Ссылка), Строка(Форма.Объект.Ссылка),
				БиблиотекаКартинок.Информация32);
