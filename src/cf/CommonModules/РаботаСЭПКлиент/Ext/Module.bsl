	...
	Для Каждого Объект Из МассивОбъектов Цикл
		...
		Если ТипЗнч(Объект) = Тип("Структура")
			...
			Если Объект.Свойство("ПараметрыДанных") Тогда
				...
				Если ПараметрыДанных.Свойство("ЗаполнятьФормализованныйЭДО")
					И Не ПараметрыДанных.Свойство("ДанныеЗаполненияФормализованногоЭДО") Тогда
		...
		Если ДелопроизводствоКлиентСервер.ЭтоДокумент(ОбъектПодписания) Тогда
			...
			Для Каждого Файл Из МассивПодчиненныхФайлов Цикл
				...
				ПараметрыДанныхФайла.Вставить("ЗаполнятьФормализованныйЭДО", Ложь);
				ПараметрыДанныхФайла.Вставить("ДанныеЗаполненияФормализованногоЭДО",
					ФорматыЭДОДокументооборотКлиентСервер.НовыеДанныеДляЗаполненияФормализованногоДокумента());
				ПараметрыДанныхФайла.Вставить("ДанныеИзменены", Ложь);
				
				Если ПараметрыДанных <> Неопределено
					И ПараметрыДанных.Свойство("ЗаполнятьФормализованныйЭДО")
					И ПараметрыДанных.ЗаполнятьФормализованныйЭДО = Истина Тогда
					
					ПараметрыДанныхФайла.ЗаполнятьФормализованныйЭДО = Истина;
					ПараметрыДанныхФайла.ДанныеЗаполненияФормализованногоЭДО =
						ПараметрыДанных.ДанныеЗаполненияФормализованногоЭДО;
...
Процедура ПриЗапросеДвоичныхДанныхОбъекта(Параметры, Контекст) Экспорт
	...
	Если ДелопроизводствоКлиентСервер.ЭтоФайл(Контекст.ПодписываемыеДанные) Тогда
		...
		ЗаполнятьФормализованныйЭДО = Ложь;
		...
		Если Контекст.Свойство("ПараметрыДанных") Тогда
			...
			Если ДополнительныеПараметры.Свойство("ЗаполнятьФормализованныйЭДО")
				...
				ЗаполнятьФормализованныйЭДО = Истина;
			КонецЕсли;
		КонецЕсли;
		
		Если ЗаполнятьФормализованныйЭДО Тогда
			
			Сертификат = Параметры.ОписаниеДанных.ВыбранныйСертификат.Ссылка;
			ДоверенностиСертификата = Параметры.ОписаниеДанных.ДоверенностиСертификатов[Сертификат];
			Доверенность = ДоверенностиСертификата[0];
			
			РезультатЗаполнения = ОбменСКонтрагентамиДОВызовСервера.ЗаполнитьДокументЭДОПриПодписании(
				Контекст.ПодписываемыеДанные,
				ДополнительныеПараметры.ДанныеЗаполненияФормализованногоЭДО,
				Сертификат,
				Доверенность);
			
			РезультатПолученияДанных.Данные = РезультатЗаполнения.ДвоичныеДанные;
			РезультатПолученияДанных.ДанныеИзменены = РезультатЗаполнения.ДанныеИзменены;
			РезультатПолученияДанных.ПричинаИзмененияДанных =
				НСтр("ru = 'Заполнен формализованный документ для отправки по ЭДО.'");
	...
	Иначе
		...
		Если ДополнительныеПараметры.Свойство("ЗаполнятьФормализованныйЭДО")
			...
			ДанныеЗаполненияФормализованногоЭДО = ДополнительныеПараметры.ДанныеЗаполненияФормализованногоЭДО;
			
			Если ДанныеЗаполненияФормализованногоЭДО.ПодписантДокумента <> Неопределено Тогда
				ДанныеЗаполненияФормализованногоЭДО.ПодписантДокумента.Сертификат = Сертификат;
				ДанныеЗаполненияФормализованногоЭДО.ПодписантДокумента.Доверенность = Доверенность;
			КонецЕсли;
...
Процедура ПриПолученииПодписи(Параметры, Контекст) Экспорт
	...
	Если Контекст.Свойство("ПараметрыДанных")
		И Контекст.ПараметрыДанных.Свойство("ЗаполнятьФормализованныйЭДО")
		...
		Если ПараметрыДанных.ЗаполнятьФормализованныйЭДО = Истина
...
Процедура ПроверитьПодписиФайловЭДО(Контекст)
	...
	ДействиеПроверки = ДействияПроверкиПодписей().ПроверитьПодписиФайловЭДО;
	Если Не НеобходимоВыполнитьДействиеПроверкиПодписей(ДействиеПроверки, Контекст) Тогда
		Возврат;
	КонецЕсли;
