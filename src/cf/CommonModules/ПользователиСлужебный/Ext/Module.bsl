...
Процедура УстановитьНачальныеНастройки(Знач ИмяПользователя, ЭтоВнешнийПользователь = Ложь) Экспорт
	...
	НастройкиИнтерфейса.ОтображениеПанелиРазделов = ОтображениеПанелиРазделов.КартинкаСлеваИТекст;
...
Процедура ОбновитьСоставыИерархическихГруппПользователей(ГруппаПользователей,
	...
	|				ТОГДА ЛОЖЬ
	|			КОГДА СотрудникиПользователей.Пользователь.ПометкаУдаления
	|				ТОГДА ЛОЖЬ
	|			КОГДА СотрудникиПользователей.Пользователь.Недействителен
	|				ТОГДА ЛОЖЬ
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ) КАК Используется
	...
	Иначе
		...
		ОтборПользователяВСправочнике = "СотрудникиПользователей.Пользователь В (&Пользователь)";
	КонецЕсли;
