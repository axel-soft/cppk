...
Функция СоздатьДокументНаОсновеФайла(
	...
	РазрешеноДобавлятьФайлыТаймлист
		= ТаймлистВызовСервера.РазрешеноДобавлятьФайлыТаймлистПревышающихМаксимальноДопустимыйОбъем(
			РасширениеФайла, РазмерВМб);
	
	Если Файл.Размер() > МаксРазмерФайла И Не РазрешеноДобавлятьФайлыТаймлист Тогда
...
Процедура ОткрытьФайлСОповещением(ОбработчикРезультата, ДанныеФайла, УникальныйИдентификатор = Неопределено) Экспорт
	...
	КлючеваяОперация = "ФайлыОткрытиеНаПросмотр";
...
Функция ОбработкаПеретаскиванияВЛинейныйСписок(
	...
	НадоПереименоватьФайлыПоШаблонуПослеДрагдропа = Ложь) Экспорт
...
Процедура НайтиСлишкомБольшиеФайлы(
	...
	Для Каждого ВыбранныйФайл Из МассивФайлов Цикл
		...
		Если ВыбранныйФайл.Существует() Тогда
			...
			РазрешеноДобавлятьФайлыТаймлист
				= ТаймлистВызовСервера.РазрешеноДобавлятьФайлыТаймлистПревышающихМаксимальноДопустимыйОбъем(
					ВыбранныйФайл.Расширение, РазмерВМб);
			
			// Размер файла слишком большой.
			Если ВыбранныйФайл.Размер() > МаксРазмерФайла И Не РазрешеноДобавлятьФайлыТаймлист Тогда
	...
	Для Каждого Файл из МассивФайлов Цикл
		...
		Иначе
			...
			Иначе
				...
				РаботаСФайламиВызовСервера.ЗаписатьРольФайла(ФайлЭлементСправочника, РольФайла);
	...
	Если ПутьКФайлуВКеше = "" Тогда
		...
		Если ПолучитьФайлы(ПередаваемыеФайлы,, ПутьКФайлу, Ложь) Тогда
			...
			Если ПротоколированиеРаботыСотрудниковКлиентПовтИсп.ПолучитьИспользоватьПротоколированиеРаботыСотрудников() Тогда
				ПротоколированиеРаботыСотрудников.ЗаписатьСохранитьКак(ДанныеФайла.Ссылка, ВыбранноеПолноеИмяФайла);
			КонецЕсли;
...
Процедура ОткрытьФайлСОповещениемПослеУстановкиРасширения(РасширениеУстановлено, ПараметрыВыполнения) Экспорт
	...
	Подключено = ФайловыеФункцииСлужебныйКлиент.РасширениеРаботыСФайламиПодключено();
...
#КонецЕсли
	...
	Если ПротоколированиеРаботыСотрудниковКлиентПовтИсп.ПолучитьИспользоватьПротоколированиеРаботыСотрудников() Тогда
		ПротоколированиеРаботыСотрудников.ЗаписатьСохранитьКак(ПараметрыВыполнения.ДанныеФайла.Ссылка, ВыборФайла.ПолноеИмяФайла);
	КонецЕсли;
