...
Процедура ЗаполнитьДополнительныеРеквизитыВФорме(Форма, Объект = Неопределено, ПоляНадписей = Ложь, СкрытьУдаленные = Неопределено) Экспорт
	...
	Для Каждого ОписаниеСвойства Из ОписаниеСвойств Цикл
					...
					Иначе
						...
						Если ЭлементРазмещения = Неопределено Тогда
							Если Пользователи.ЭтоПолноправныйПользователь() Тогда
								Сообщить(СтрШаблон(
									НСтр("ru = 'Не найдена группа элементов формы ""%1"" для расположения реквизита ""%2""'"),
									ПутьГруппы[1],
							...
							Иначе
								ЗаписьЖурналаРегистрации(
									...
									СтрШаблон(НСтр("ru = 'Не найдена группа элементов формы ""%1"" для расположения реквизита ""%2""'"),
										ПутьГруппы[1],
										ОписаниеСвойства.Наименование));
							КонецЕсли;	
						КонецЕсли;
		...
		Если ОписаниеСвойства.ВыводитьВВидеГиперссылки Тогда
			...
			Если ЭлементОриентир <> Неопределено И ЭлементОриентир <> Истина Тогда
				ГруппаГиперссылки = Форма.Элементы.Вставить(ИмяГруппыГиперссылки, Тип("ГруппаФормы"), Родитель, ЭлементОриентир);
			Иначе
	...
	Если Форма.ОбъектПоддерживаетПроверкуУсловийДляДопСвойств Тогда
		Форма.ДопРеквизитыОтбораДляУсловий = УправлениеСвойствамиДокументооборот.ДопРеквизитыОтбораДляУсловий(ОписаниеОбъекта);
...
Функция ПредставлениеЗначения(Значение, КодЯзыка, ФорматСвойства)
	...
	Иначе
		...
		ЕстьРеквизитНаименование = Метаданные.Справочники.Содержит(ОбъектМетаданных)
			И ОбъектМетаданных.ОсновноеПредставление
				= Метаданные.СвойстваОбъектов.ОсновноеПредставлениеСправочника.ВВидеНаименования
			Или Метаданные.ПланыВидовРасчета.Содержит(ОбъектМетаданных)
			И ОбъектМетаданных.ОсновноеПредставление
				= Метаданные.СвойстваОбъектов.ОсновноеПредставлениеВидаРасчета.ВВидеНаименования
			Или Метаданные.ПланыВидовХарактеристик.Содержит(ОбъектМетаданных)
			И ОбъектМетаданных.ОсновноеПредставление
				= Метаданные.СвойстваОбъектов.ОсновноеПредставлениеВидаХарактеристики.ВВидеНаименования
			Или Метаданные.ПланыСчетов.Содержит(ОбъектМетаданных)
			И ОбъектМетаданных.ОсновноеПредставление
				= Метаданные.СвойстваОбъектов.ОсновноеПредставлениеСчета.ВВидеНаименования
			Или Метаданные.ПланыОбмена.Содержит(ОбъектМетаданных)
			И ОбъектМетаданных.ОсновноеПредставление
				= Метаданные.СвойстваОбъектов.ОсновноеПредставлениеПланаОбмена.ВВидеНаименования
			Или Метаданные.Задачи.Содержит(ОбъектМетаданных)
			И ОбъектМетаданных.ОсновноеПредставление
				= Метаданные.СвойстваОбъектов.ОсновноеПредставлениеЗадачи.ВВидеНаименования;
