...
Функция РазложитьПолноеИмяФайла(Знач ПолноеИмяФайла, ЭтоКаталог = Ложь) Экспорт
	...
	ПозицияПоследнегоРазделителяПути = СтрНайти(ПолноеИмяФайла, "\", НаправлениеПоиска.СКонца);
	Если ПозицияПоследнегоРазделителяПути = 0 Тогда
		ПозицияПоследнегоРазделителяПути = СтрНайти(ПолноеИмяФайла, "/", НаправлениеПоиска.СКонца);
	КонецЕсли;
	
	ИмяФайла = "";
	Если ПозицияПоследнегоРазделителяПути <> 0 Тогда
		ИмяФайла = Сред(ПолноеИмяФайла, ПозицияПоследнегоРазделителяПути + 1);
		
		РазделительПути = Сред(ПолноеИмяФайла, ПозицияПоследнегоРазделителяПути, 1);
		РазделенныйПуть =
			СтрРазделить(Лев(ПолноеИмяФайла, ПозицияПоследнегоРазделителяПути - 1), РазделительПути, Ложь);
		СтруктураФайла.Путь = СтрСоединить(РазделенныйПуть, РазделительПути) + РазделительПути;
	Иначе
		ИмяФайла = ПолноеИмяФайла;
	КонецЕсли;
	
	СтруктураИмениФайла = РазложитьИмяФайла(ИмяФайла);
	ЗаполнитьЗначенияСвойств(СтруктураФайла, СтруктураИмениФайла);	
	
	СтруктураФайла.ПолноеИмя = СтруктураФайла.Путь + СтруктураФайла.Имя;
...
Функция РазложитьИмяФайла(Знач ИмяФайла) Экспорт
	...
	СтруктураИмениФайла = Новый Структура("Имя, Расширение, ИмяБезРасширения", "", "", "");
	СтруктураИмениФайла.Имя = ИмяФайла;
	
	ПозицияТочки = СтрНайти(СтруктураИмениФайла.Имя, ".", НаправлениеПоиска.СКонца);
	Если ПозицияТочки <> 0 Тогда
		СтруктураИмениФайла.Расширение = Сред(СтруктураИмениФайла.Имя, ПозицияТочки);
