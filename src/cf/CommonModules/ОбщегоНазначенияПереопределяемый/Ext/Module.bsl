...
Процедура ПриДобавленииПараметровРаботыКлиентаПриЗапуске(Параметры) Экспорт
	...
	ОбновитьКонтроли =
		Пользователи.ЭтоПолноправныйПользователь()
		И Не Обработки.ОбновлениеДанныхКонтроля.ОтложенноеОбновлениеЗавершено();
	Параметры.Вставить("ОбновитьКонтроли", ОбновитьКонтроли);
...
Процедура ПослеЗаменыСсылок(Результат, ПараметрыВыполнения, ТаблицаПоиска) Экспорт
	...
	Если Результат.ЕстьОшибки Тогда
		...
		ТекстИсключения = НСтр("ru = 'Ошибка замены ссылок:'");
		
		Для Каждого Ошибка Из Результат.Ошибки Цикл
			ТекстИсключения = ТекстИсключения + СтрШаблон(НСтр("ru = '%1Объект: %2 Тип: %3 Текст: %4'"),
				Символы.ПС, Ошибка.ПредставлениеОбъектаОшибки, Ошибка.ТипОшибки, Ошибка.ТекстОшибки);
		КонецЦикла;
	...
	Если Не Результат.ОчередьКНепосредственномуУдалению.Количество() Тогда
		Возврат;
	КонецЕсли;
	
	УдаляемаяСсылка = Результат.ОчередьКНепосредственномуУдалению[0];
	
	ИдентификаторОбъекта = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(ТипЗнч(УдаляемаяСсылка));
	ОписаниеКлюча = КОДСервер.КлючДанныхВФорматКлючаОтметокВремени(УдаляемаяСсылка, ИдентификаторОбъекта);
	
	СтарыйНаборЗаписей = РегистрыСведений.НаличиеДанныхВУзлахКОД.СоздатьНаборЗаписей();
	СтарыйНаборЗаписей.Отбор.ИдентификаторКлюча.Установить(ОписаниеКлюча.ИдентификаторКлюча);
	СтарыйНаборЗаписей.Отбор.ТипКлюча.Установить(ОписаниеКлюча.ТипКлюча);
	СтарыйНаборЗаписей.Отбор.Объект.Установить(ИдентификаторОбъекта);
	...
	СтарыйНаборЗаписей = РегистрыСведений.ОтметкиРассмотренияСсылочныхДанныхКОД.СоздатьНаборЗаписей();
	СтарыйНаборЗаписей.Отбор.ИдентификаторКлюча.Установить(ОписаниеКлюча.ИдентификаторКлюча);
	СтарыйНаборЗаписей.Отбор.ТипКлюча.Установить(ОписаниеКлюча.ТипКлюча);
	СтарыйНаборЗаписей.Прочитать();
	Если СтарыйНаборЗаписей.Количество() Тогда
	...
	СтарыйНаборЗаписей = РегистрыСведений.ОтметкиВремениПолученныхСсылочныхДанных.СоздатьНаборЗаписей();
	СтарыйНаборЗаписей.Отбор.ИдентификаторКлюча.Установить(ОписаниеКлюча.ИдентификаторКлюча);
	СтарыйНаборЗаписей.Отбор.ТипКлюча.Установить(ОписаниеКлюча.ТипКлюча);
	СтарыйНаборЗаписей.Прочитать();
	Если СтарыйНаборЗаписей.Количество() Тогда
	...
	СтарыйНаборЗаписей = РегистрыСведений.ОтметкиВремениСсылочныхОбъектов.СоздатьНаборЗаписей();
	СтарыйНаборЗаписей.Отбор.ИдентификаторКлюча.Установить(ОписаниеКлюча.ИдентификаторКлюча);
	СтарыйНаборЗаписей.Отбор.ТипКлюча.Установить(ОписаниеКлюча.ТипКлюча);
	СтарыйНаборЗаписей.Прочитать();
	Если СтарыйНаборЗаписей.Количество() Тогда
