...
Функция ПодготовитьПодключение(СвойстваПодключения, СертификатКлиента = Неопределено, ПараметрыОперации = Неопределено) Экспорт
	...
	Попытка
		...
		Если СертификатКлиента = Неопределено 
			...
			ТипСоединения = ЭлектроннаяПодписьДокументооборотКлиентСервер.НовоеЗащищенноеСоединение(Неопределено, СертификатыУЦ,
				?(СвойстваПодключения.ЗащищенноеСоединениеГОСТ, "КриптоПро", "OpenSSL"));
		ИначеЕсли СвойстваПодключения.ЗащищенноеСоединение Тогда	
			ТипСоединения = ЭлектроннаяПодписьДокументооборотКлиентСервер.НовоеЗащищенноеСоединение(СертификатКлиента, Неопределено,
				?(СвойстваПодключения.ЗащищенноеСоединениеГОСТ, "КриптоПро", "OpenSSL"));
		КонецЕсли;
