...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбменСКонтрагентамиПереопределяемый.ПриПроверкиГотовностиЭлектронногоДокумента(Объект, ТолькоПросмотр);
...
Процедура ПодготовитьОтражениеВУчете()
	...
	Если (ЭлементСхемы.ТипДокумента = Перечисления.ТипыДокументовЭДО.УПД 
		Или ЭлементСхемы.ТипДокумента = Перечисления.ТипыДокументовЭДО.УКД) 
		И СписокСоздания.Количество() = 0 Тогда 
		
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Не установлены способы отражения универсального документа в
		|ОбменСКонтрагентамиПереопределяемый.СпособыОтраженияВУчетеТипаЭлектронногоДокумента.
...
Процедура ПоказатьВыборСозданияДокументаУчета()
	...
	Если СписокСоздания.Количество() = 0 Тогда
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru='Не установлены способы отражения документа.
		|Пожалуйста обратитесь к администратору!'"));
		Возврат;
